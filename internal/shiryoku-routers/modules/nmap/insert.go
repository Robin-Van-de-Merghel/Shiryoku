...
func InsertNmapScans(nmapDB osdb.NmapDBIface) func(c *gin.Context) {
	return func(c *gin.Context) {
		var nmapDataList []models.NmapData

		if err := c.ShouldBindJSON(&nmapDataList); err != nil {
			utils.ParseJSONError(c, err)
			return
		}

		ids, err := logicnmap.SaveNmapScans(c.Request.Context(), nmapDataList, nmapDB)
		if err != nil {
			c.JSON(500, gin.H{"error": err.Error()})
			return
		}
