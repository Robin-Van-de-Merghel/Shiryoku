...
func NewPostgresDB(dsn string) (*gorm.DB, error) {
	...
	if err := db.Exec(`
		...
		ON nmap_services(service_name, service_product, service_version, service_extra_info, protocol, service_tunnel)
	...
	if err := db.Exec(`
		...
		ON nmap_scan_results(scan_id, host_id, service_id, port)
	`).Error; err != nil {
